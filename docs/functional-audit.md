...
| Vínculo operador-condomínio | Implementado | `lib/operator-auth.ts`, `supabase-schema.sql` |
...
| Notificação ao morador por e-mail | Implementado com dependência externa | `app/actions.ts`, `lib/email/client.ts` |
| Aviso com foto e QR | Implementado | `app/actions.ts`, `lib/evolution/client.ts`, `lib/email/client.ts` |
...
- o condomínio ativo precisa estar dentro das memberships do operador
...
- o envio de e-mail depende de `Resend` (`RESEND_API_KEY` e `EMAIL_FROM`)
